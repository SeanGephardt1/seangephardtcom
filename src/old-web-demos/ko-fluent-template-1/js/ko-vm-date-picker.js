...
function DatePickerViewModel( parent )
{
	...
	{	//	console.debug( "MarkDisplayCalenderViews" );
		...
		if ( _self.CurrentSelectedRange().length > 0 )
		{
		...
		//_today.getTime();
		//	console.debug( "_self.CurrentSelectedRange()[0].Date() ", _self.CurrentSelectedRange()[0].Date().getTime() );
		//	console.debug( "_self.CurrentSelectedRange()[_self.CurrentSelectedRange().length - 1].Date(", _self.CurrentSelectedRange()[_self.CurrentSelectedRange().length - 1].Date() );

		if ( _self.CurrentSelectedRange()[0].Date().getTime() < _self.CurrentDate().getTime() )
		{
			let _has_selection = false;
		...
		if ( _self.CurrentSelectedRange()[_self.CurrentSelectedRange().length - 1].Date().getTime() > _self.CurrentDate().getTime() )
		{
			let _has_selection = false;
			_self.ForecastButtons().forEach( function ( v, i, a )
			{	//	console.debug( i, v.ButtonText(), v.IsSelected() );
				if ( i < a.length - 1 )
				{
					if ( v.IsSelected() == true )
	...
	{	//	
		//	console.debug( "OnClick_SelectThisDay ", vm.Date(), vm.IsSelected(), _self.CurrentSelectedRange().length);
	...
	this.OnClick_ForecastNone = function ( vm, ev )
	{	//	console.debug( "OnClick_ForecastNone", _self.CurrentSelectedRange().length, "CurrentDate", _self.CurrentDate() );
		_self.OnClick_ClearSelectionValues();
		...
		_self.CurrentSelectedRange()[0] = _today;
		_self.CurrentSelectedRange()[1] = _today;

		_self.MarkDisplayCalenderViews();
		return false;
	};
	this.OnClick_ForecastEOM = function ( vm, ev )
	{
		//	console.debug( "OnClick_ForecastEOM", _self.CurrentSelectedRange().length, "CurrentDate", _self.CurrentDate() );
		//	_self.OnClick_ClearSelectionValues();
		...
		let _eom_date = new Date( _self.CurrentDate().getFullYear(), _self.CurrentDate().getMonth() + 1, 0 );
		//	console.debug( "_eom_date", _eom_date );

		let _today_plus_eom = new WeekDayViewModel( _self, _eom_date );
		//	console.debug( "_today_minus_range", _today_minus_range );
		...
		_self.CurrentSelectedRange()[1] = _today_plus_eom ;

		_self.MarkDisplayCalenderViews();
		return false;
	};
	this.OnClick_ForeCastOneMonth = function ( vm, ev )
	{
		//	console.debug( "OnClick_OneMonth", _self.CurrentSelectedRange().length, "CurrentDate", _self.CurrentDate() );
		//	_self.OnClick_ClearSelectionValues();
		...
		let _eom_date = new Date( _self.CurrentDate().getFullYear(), _self.CurrentDate().getMonth() + 2, 0 );
		//	console.debug( "_eom_date", _eom_date );

		let _today_plus_eom = new WeekDayViewModel( _self, _eom_date );
		//	console.debug( "_today_plus_eom", _today_plus_eom );
	...
	this.OnClick_ForecastEndOfQuarter = function ( vm, ev )
	{	//	console.debug( "OnClick_EndOfQuarter", _self.CurrentSelectedRange().length, "CurrentDate", _self.CurrentDate() );
		_self.ForecastButtons().forEach( function ( v, i, a )
		{	//	console.debug( i, v.ButtonText(), v.IsSelected() );
			v.IsSelected( false ); 
			return;
		...
		let _end_quarter =  new Date( _year, _quarter_month, 0 );
		//	console.debug( "_end_quarter", _end_quarter );
		let _end = new WeekDayViewModel( _self, _end_quarter );
	...
	this.OnClick_ForecasterNextThreeMonths = function ( vm, ev )
	{	//	console.debug( "OnClick_ForecasterNextThreeMonths", _self.CurrentSelectedRange().length, "CurrentDate", _self.CurrentDate() );
		_self.ForecastButtons().forEach( function ( v, i, a )
		{	//	console.debug( i, v.ButtonText(), v.IsSelected() );
			v.IsSelected( false ); 
			return;
		...
		let _end_date = new Date( _self.CurrentDate().getFullYear(), _self.CurrentDate().getMonth() + 3, _self.CurrentDate().getDate() );
		//	console.debug( "_end_date", _end_date );
		let _end = new WeekDayViewModel( _self, _end_date );
	...
	this.OnClick_ForecasterEndOfYear = function ( vm, ev )
	{	//	console.debug( "OnClick_ForecasterEndOfYear", _self.CurrentSelectedRange().length, "CurrentDate", _self.CurrentDate() );
		_self.ForecastButtons().forEach( function ( v, i, a )
		{	//	console.debug( i, v.ButtonText(), v.IsSelected() );
			v.IsSelected( false ); 
			return;
		...
		let _end_date = new Date( _self.CurrentDate().getFullYear(), 12, 0 );
		//	console.debug( "_end_date", _end_date );
		let _end = new WeekDayViewModel( _self, _end_date );
	...
	this.ForecastButtons = ko.observableArray(
		[
			new RangeButton( "None", false, this.OnClick_ForecastNone ),
			new RangeButton( "EOM", false, this.OnClick_ForecastEOM),
			new RangeButton( "1 M", false, this.OnClick_ForeCastOneMonth ),
			new RangeButton( "EOQ", false, this.OnClick_ForecastEndOfQuarter ),
			new RangeButton( "3 M", false, this.OnClick_ForecasterNextThreeMonths ),
			new RangeButton( "EOY", false, this.OnClick_ForecasterEndOfYear ),
			new RangeButton( "Custom", false, this.OnClick_HistoricCustom ),
		] );
