...
import { SvgIconControl as SvgIcon } from "./../svg-icons/svg-icon.js";
...
//	rgba(52, 144, 220, 1)
//	rgba(255,255,255,1)
class AzureLogoAppLoaderControl extends React.Component
{
	constructor( props )
	{	
		super( props );
		console.debug( "AzureLogoAppLoaderControl.ctor()", props );
	...
	{	//	console.debug( "AppLoader.componentDidMount()");
		...
		{	//	console.debug( "AppLoader.componentDidMount()::this.AnimationID", this.AnimationID );
			this.AnimationID = window.requestAnimationFrame( () => this.DoLogoAnimation() );
		}
		return;
	}
	...
	render()
	{	
		//	console.debug( "AzureLogoAppLoaderControl.render()", this.state.leftValue, this.state.rightValue );	
			//<path id="left-ani" fill="rgba(52, 144, 220, 1)" d="M 283 57 146 170 36 360 148 360 Z"></path>
			//<path id="right-ani" fill="rgba(52, 144, 220, 1)" d="M 165 386 185 390 473 392 288 80 234 232 342 361 342 361 Z"></path>

		return (
			<div className="app-loader-panel">
				{
					( this.props.start === true && this.props.iterations > 0 ) &&
					<svg viewBox="0 0 500 500" x="0px" y="0px" width="100%" height="100%" style={{ backgroundColor: 'rgba(255, 255, 255, 1)' }}>
						<path fill="rgba(52, 144, 220, 1)" d={this.state.leftValue}></path>
						<path fill="rgba(52, 144, 220, 1)" d={this.state.rightValue}></path>
					</svg>						
				}
				{
					( this.props.start === true  && this.props.iterations < 1) &&
					<svg viewBox="0 0 500 500" x="0px" y="0px" width="100%" height="100%" style={{ backgroundColor: 'rgba(255, 255, 255, 1)', cursor: 'pointer' }}
						onClick={this.props.stopEvent}>
				...
				<div className="msft-logo">
					{/* if fabric prog line control is enabled in config*/}
					{
						( this.props.config.AzureLoader.ProgressLine === true && this.props.config.AzureLoader.Spinner === false ) &&
					...
					{
						( this.props.config.AzureLoader.ProgressLine === false && this.props.config.AzureLoader.Spinner === true ) &&
...
export
{
	AzureLogoAppLoaderControl as AzureLogoSpinner
};