...
export default class Html5CanvasDemo extends React.Component
{
	...
	{
		fillGrad.addColorStop( "0", "rgba(39, 93, 173, 0.2)" );
		fillGrad.addColorStop( "0.3", "rgba(197, 34, 51, 0.2)" );
		fillGrad.addColorStop( "0.7", "rgba(248, 216, 0, 0.2)" );
		fillGrad.addColorStop( "1", "rgba(0,96,0, 0.2)" );
		return;
	}
	OnClick_ResetCanvas()
	{
		...
		//	horizontal, in order
		let fillGrad = c.createLinearGradient( 0, 0, 0, this.state.CanvasSize );
		this.DefaultGradientColorStops( fillGrad );
		c.fillStyle = fillGrad;
		c.fillRect( 0, 0, this.state.CanvasSize, this.state.CanvasSize );

		//	horizonatal, reverse order
		fillGrad = c.createLinearGradient( 0, this.state.CanvasSize , 0, 0);
		this.DefaultGradientColorStops( fillGrad );
		c.fillStyle = fillGrad;
		c.fillRect( 0, 0, this.state.CanvasSize, this.state.CanvasSize );

		//	vertical, in order
		fillGrad = c.createLinearGradient( 0, 0, this.state.CanvasSize, 0 );
		this.DefaultGradientColorStops( fillGrad );
		c.fillStyle = fillGrad;
		c.fillRect( 0, 0, this.state.CanvasSize, this.state.CanvasSize );

		//	vertical, reverse order
		fillGrad = c.createLinearGradient( this.state.CanvasSize, 0, 0, 0 );
			this.DefaultGradientColorStops( fillGrad );
		c.fillStyle = fillGrad;
		c.fillRect( 0, 0, this.state.CanvasSize, this.state.CanvasSize );

		//// diagonal, top left to bottom right
		fillGrad = c.createLinearGradient( 0, 0, this.state.CanvasSize, this.state.CanvasSize );
		this.DefaultGradientColorStops( fillGrad );
		c.fillStyle = fillGrad;
		c.fillRect( 0, 0, this.state.CanvasSize, this.state.CanvasSize );

		//// diagonal, top right to bottom left
		fillGrad = c.createLinearGradient( this.state.CanvasSize, 0, 0, this.state.CanvasSize );
		this.DefaultGradientColorStops( fillGrad );
		c.fillStyle = fillGrad;
		c.fillRect( 0, 0, this.state.CanvasSize, this.state.CanvasSize );

		//// diagonal, bottom right to top left
		fillGrad = c.createLinearGradient( this.state.CanvasSize, this.state.CanvasSize, 0, 0 );
		this.DefaultGradientColorStops( fillGrad );
		c.fillStyle = fillGrad;
		c.fillRect( 0, 0, this.state.CanvasSize, this.state.CanvasSize );

		//// diagonal, bottom left to top right
		fillGrad = c.createLinearGradient( 0, this.state.CanvasSize, this.state.CanvasSize, 0 );
		this.DefaultGradientColorStops( fillGrad );
		c.fillStyle = fillGrad;
		c.fillRect( 0, 0, this.state.CanvasSize, this.state.CanvasSize );
	...
	OnChange_SelectAnimationFromList( ev )
	{
		...
		{
			case 0: {
				selected = this.AnimationsList[0];
				break;
			}
			case 1: {
				selected = this.AnimationsList[1];
				break;
			}
			case 2: {
				selected = this.AnimationsList[2];
				break;
			}
			default: {	// is this really needed
				selected = this.AnimationsList[0];
				break;
			}
		}
